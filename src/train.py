...
from src.config import (
    PACKED_DIR, PACKED_FILES, META_FILE,
    SEED, DEVICE, DTYPE, COMPILE,
...
def configure_optimizers(model: GPT, weight_decay: float, lr: float,
...
def train() -> None:
    ...
    if RESUME and last_path.exists():
        model, ckpt = load_checkpoint(last_path, device)
        cfg = model.cfg
        optimizer = configure_optimizers(model, WEIGHT_DECAY, LR, (BETA1, BETA2), device)
        optimizer.load_state_dict(ckpt["optimizer"])
        start_step = ckpt["step"] + 1
        best_val = ckpt["best_val_loss"]
        print(f"resumed from {last_path} at step {start_step} (best_val {best_val:.4f})")
    else:
